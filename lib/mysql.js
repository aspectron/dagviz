...
class mySQL {
			...
			if(!fs.existsSync(path.join(this.dataFolder,'mysql'))) {
				...
				dpc(MYSQL_STARTUP_DELAY, () => {
					...
					if(maria) {
						mysqlInitArgs.push(
							...
							//darwin //`"UPDATE mysql.user SET authentication_string=PASSWORD('dagviz')  WHERE  User='root';GRANT ALL PRIVILEGES ON *.* TO root@localhost; FLUSH PRIVILEGES;"`
						);
					} else {
				...
				dpc(1000, () => {
					...
					db.connect(async (err) => {
						...
						if(err) {
							this.log(err);
							this.log("Unable to connect to MySQL database... (db init)".brightRed);
							return reject(err);
						}

						this.log("MySQL started...".brightGreen);
	...
	stop() {
		...
		return new Promise((resolve,reject)=>{
			...
			console.error("MySQL shutdown: error", e)
		})
	}
